...
def train_model(model, model_name, train_loader, test_loader, device, epochs, results_dir):
    ...
    for epoch in range(epochs):
        ...
        # === 保存 CAM 和 mask-overlay 可视化图 ===
        vis_dir = os.path.join(results_dir, "vis")
        os.makedirs(vis_dir, exist_ok=True)

        # 可视化目标图像列表
        # 根据当前训练图像动态选择可视化目标
        # train_vis_img = train_img_name
        # val_vis_img = "1.png" if train_img_name == "8.png" else "8.png"
        val_vis_img = "17.png"
        ...
        for vis_img_name, vis_tag in vis_targets:
            ...
            try:
                ...
                raise ValueError("请确认 model.decoder.blocks[-1].conv1 是正确的 GradCAM 层")
...
def metrice(y_true, y_pred):
    """计算 mPA, mDice, mIoU，同时增加 Recall 和 Precision 只针对前景（单类别）"""
    cm = confusion_matrix(y_true.flatten(), y_pred.flatten(), labels=[0, 1])  # 二元混淆矩阵，labels 确保只针对背景(0)和前景(1)

    # 计算 mPA（Mean Pixel Accuracy）
